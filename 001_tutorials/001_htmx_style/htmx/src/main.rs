...
async fn home() -> Result {
    view! {
        ...
        // A `<script>` body is parsed as view nodes, not raw text, so the JS
        // is wrapped in `Unescaped` (trusted markup emitted verbatim) rather
        // than a string literal, which would HTML-escape `<`, `>` and `&`.
        <script>
            (Unescaped::new_unchecked(r##"
